// Purpose: Unit tests for core logic with mocks; fast and CI-friendly.
// Notes: Mocks Playwright/Sharp/process; no real browser, server, or I/O.
/**
...
describe("GitHub Screenshot Action", () => {
  ...
  test("Generates correct output filenames when not enough are provided", () => {
    ...
    expect(generateOutputFiles(["url1", "url2", "url3"], ["output1.png"])).toEqual([
      "output1.png",
      "output1_2.png",
      "output1_2_3.png",
    ]);
  ...
  test("Starts servers on the correct ports", () => {
    ...
    function startLocalServers(localPorts, workspacePath) {
      ...
      for (const port of localPorts) {
        servers[port] = spawn("npx", ["serve", workspacePath, "--listen", `${port}`], {
          stdio: "inherit",
          shell: true,
        });
      }
    ...
    expect(spawn).toHaveBeenCalledWith("npx", ["serve", "/workspace", "--listen", "8080"], {
      stdio: "inherit",
      shell: true,
    });
  ...
  test("Full screenshot workflow integration", async () => {
    ...
    let result = await runScreenshotWorkflow(["https://example.com"], ["/workspace/screenshot.webp"], 1280, 800);
