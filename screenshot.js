...
const ensureDir = async dir => fs.mkdir(dir, { recursive: true }).catch(e => e.code === 'EEXIST' ? null : Promise.reject(e));
...
(async () => {
  ...
  const screenshotPairs = (process.env.SCREENSHOTS || "/=screenshot.webp").split(',').filter(Boolean)
    .map(pair => {
  ...
  const options = {
    webp: process.env.INPUT_WEBP_OPTIONS ? JSON.parse(process.env.INPUT_WEBP_OPTIONS) : { lossless: true, quality: 100 },
